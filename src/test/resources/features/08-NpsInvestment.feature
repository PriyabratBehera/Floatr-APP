...
#    And user go back from "NPS" screen
  ...
  Scenario: Validate UI Elements on NPS Landing Page
    ...
    And user verify static text "Get tax exemption at the time of investing.  Up to ₹1,50,000 u/s 80CCD (1) & ₹50,000 u/s 80CCD (1B) are exempted from income tax"
    And user verify static text "Zero Tax on Maturity Amount"
    And user verify static text "100% of the maturity corpus is tax free. Withdraw 60% of the amount and buy annuity of the 40% for a regular pension"
    And user verify static text "Tax Exemption on Employer Contribution"
    And user verify static text "Additional tax exemptions under section 80CCD (2) on employer contribution of upto 10% of your basic salary"
    When user scroll to view more about NPS
    Then user verify static text "KNOW ABOUT NPS"
    And user verify static text "Why to Invest in NPS"
    And user verify static text "Pension from NPS"
  ...
  Scenario: Verify Information in "More about NPS" Section
    ...
    And user verify static text "At Retirement"
    And user verify static text "Withdrawal Phase"
    ...
    And user verify static text "Power of Compounding"
...
#    And user verify static text "75%"
  ...
  Scenario: Calculate Retirement Plan
    ...
    Then user verify static text "₹4,85,62,335"
  ...
  Scenario: Verify behaviour of DEF Technologies company
    ...
    And user verify text "HDFC PENSION MANAGEMENT COMPANY LIMITED"
    And user not able to select investment fund manager
...
#    Given user login with user "7272727272 | 7272727272"
    ...
    Then user click by text "SBI PENSION FUNDS PRIVATE LIMITED"
    And user wait for "2000" milli second
...
#    Given user login with user "7272727272 | 7272727272"
    ...
    And user click by text "Rabindra, BROTHER"
#    Then verify validation error "Maximum 2 nominees allowed"
  ...
  Scenario: Verify UI Of Initial Contribution Screen
    ...
    And user verify static text "₹1,008.40"
    ...
    And user click by text "Continue"
    Then user verify partial static text "Amount should not be less than 500"

  @Validation @NPSValidation  @spaceIssueRegisterAsCorporateSub
  Scenario: Validate in Crop detail screen
    Given user login with user "8528528522 | 9696850808"
    And application launched successfully
    Then user click on NPS Investment
    And user wait for "7000" milli second
    When user click on open nps account
  ...
  @Validation @NPSValidation @NPSRegression
  Scenario: Limit on Adding More than Three Banks
    ...
    When user enter account number in bank as "123456789345" account screen
  ...
  Scenario: Add Nominee to NPS Account
    ...
    And user click by text "Continue"
    And user wait for "3000" milli second
    And user select pension fund manager as "KOTAK MAHINDRA PENSION FUND LIMITED"
    ...
    And user click by text "Continue"
    And user wait for "3000" milli second
    Then user verify text "Tahaliprasad, FATHER"

  @Smoke @NPSSmoke @NPSRegression
  Scenario: Registration as Corporate Subscriber For NPS Account
    Given user delete nps account "9812121212 | 9812121212"
    And user login with user "9812121212 | 9812121212"
    And application launched successfully
    Then user click on NPS Investment
    And user wait for "3000" milli second
    And user click on open nps account
...
#    Then user verify successful nps registration transaction as "₹1,007.50"
...
#    Given user login with user "9812121212 | 9812121212"
  ...
  Scenario: Unsuccessful Registration as Individual Subscriber for NPS Account
    ...
    And user click by text "Continue"
    And user wait for "5000" milli second
    And user enter contribution amount as "550"
    And user click static text "Declaration under the Prevention of Money Laundering Act 2002  "
    And user click by text "Continue"
...
#    And user wait for "15000" milli second
    ...
    And user wait for "2000" milli second
    When user click history icon in nps home screen
    Then user verify successful nps registration transaction as "₹1,057.4"
...
#    And user run the batch api
  ...
  Scenario: User fail the invest in nps
    ...
    When user verify failure transaction as "₹5,578.40"
    When user click by text "Sort By"
    When user click by text "Tier 1"
    Then user verify failure transaction as "₹5,578.40"
    When user click by text "Sort By"
  ...
  Scenario: Reviewing Contribution Transaction History
    ...
    And user verify static text "Invested"
    And user verify static text "Growth"
    And user verify static text "HDFC Pension Management Company Limited Scheme E - Tier I"
    And user verify static text "Allocation 75.00%"
    And user verify static text "Current "
    And user verify static text "Invested ₹1,21,426.92"
    And user verify static text "Growth +₹88,385.75"
    And user verify static text "Returns +72.79%"
    And user verify static text "Units 6238.69"
    And user verify static text "NAV ₹35.23 (22 Nov 2023)"
  ...
  Scenario: PRAN Shifting from Individual to Individual
    ...
    And user verify static text "Your PRAN shifting is under process"
    And user verify partial static text "Acknowledgement No:"
